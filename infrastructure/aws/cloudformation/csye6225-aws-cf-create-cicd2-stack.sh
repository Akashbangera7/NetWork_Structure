...
echo "Enter the name of stack for deployment group"
  ...
  read instanceec
  create_stack1=$(aws cloudformation create-stack --stack-name $stack_name2 --template-body file://csye6225-cf-cicd2.json --capabilities CAPABILITY_NAMED_IAM --parameters ParameterKey=InstanceEC,ParameterValue=$instanceec)  
if [ $? -eq 0 ]; then
