...
echo "Enter the name of stack for deployment group"
  ...
  read instanceec
  echo " "
  echo "Enter the service role ARN for CodeDeployServiceRole"
  read servicerole
  create_stack1=$(aws cloudformation create-stack --stack-name $stack_name2 --template-body file://csye6225-cf-cicd2.json --capabilities CAPABILITY_NAMED_IAM --parameters ParameterKey=InstanceEC,ParameterValue=$instanceec ParameterKey=ServiceRole,ParameterValue=$servicerole)  
if [ $? -eq 0 ]; then
